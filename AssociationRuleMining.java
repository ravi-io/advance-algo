...
public class AssociationRuleMining {
    ...
    static List<Set<String>> data = new ArrayList<>();
    static Map<Set<String>, Integer> freq = new HashMap<>();

    static int minSup = 3;
    static double minConf = 0.60;

    // Support count
    static int support(Set<String> set) {
        int count = 0;
        for (Set<String> t : data)
            if (t.containsAll(set)) count++;
        return count;
    }

    // Generate candidates
    static List<Set<String>> candidates(List<Set<String>> prev, int k) {
        Set<Set<String>> result = new HashSet<>();

        for (int i = 0; i < prev.size(); i++)
            for (int j = i + 1; j < prev.size(); j++) {

                Set<String> c = new TreeSet<>(prev.get(i));
                c.addAll(prev.get(j));

                if (c.size() == k && allSubsets(c, prev))
                    result.add(c);
            }
    ...
    // Apriori pruning
    static boolean allSubsets(Set<String> c, List<Set<String>> prev) {
        for (String x : c) {
    ...
    static List<Set<String>> frequent(List<Set<String>> cs) {
    ...
    static void rules() {
        ...
        for (Set<String> item : freq.keySet()) {

            if (item.size() < 2) continue;

            int unionCount = freq.get(item);

            for (String x : item) {

                Set<String> a = new TreeSet<>(item);
                a.remove(x);

                Set<String> b = new TreeSet<>();
                b.add(x);

                int aCount = freq.getOrDefault(a, 0);
                int bCount = freq.getOrDefault(b, 0);

                if (aCount == 0 || bCount == 0) continue;

                double confidence =
                        (double) unionCount / aCount;

                double support =
                        (double) unionCount / data.size();

                double lift =
                        confidence /
                        ((double) bCount / data.size());

                if (confidence >= minConf)
                    System.out.printf(
                        "%s => %s | Support: %.2f | Confidence: %.2f | Lift: %.2f%n",
    ...
    public static void main(String[] args) {
        ...
        // Transactions
        data.add(new HashSet<>(Arrays.asList("Milk","Bread","Butter")));
        data.add(new HashSet<>(Arrays.asList("Milk","Bread")));
        data.add(new HashSet<>(Arrays.asList("Milk","Diaper","Beer","Bread")));
        data.add(new HashSet<>(Arrays.asList("Bread","Butter")));
        data.add(new HashSet<>(Arrays.asList("Milk","Diaper","Beer")));
        data.add(new HashSet<>(Arrays.asList("Milk","Bread","Butter","Diaper")));
        ...
        while (!current.isEmpty()) {
            ...
            k++;
            current = frequent(candidates(current, k));
        }
